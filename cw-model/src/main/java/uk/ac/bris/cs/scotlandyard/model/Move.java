 ...
 */
// 每次的移动，有两个实现类，double和single
	// 用了访问者模式，这个方法允许你对不同类型的 Move 执行不同的操作，而不需要修改 Move 类本身。
//通过实现一个 Visitor<T>，你可以对 SingleMove 和 DoubleMove 采取不同的处理策略。这个模式让你可以在不修改 Move 类的情况下，添加更多的处理方式。
	//你可以实现 Visitor 接口，定义在访问每种移动类型时该做什么。例如，如果你想计算玩家在每次移动后的分数或更新其他状态，就可以使用这个访问者模式。
